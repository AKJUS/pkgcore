...
class TestPackage(TestBase):
    ...
    def test_flag_with_restrict(self, attr):
        ...
        assert getattr(mk_pkg(repo_objs.FlagWithRestrict(value=False, restrict=None)), attr) is False
        assert getattr(mk_pkg(repo_objs.FlagWithRestrict(value=True, restrict=None)), attr) is True

        # restrict that matches the package
        match = repo_objs.FlagWithRestrict(value=True, restrict=atom.atom(">dev-util/diffball-0"))
        assert getattr(mk_pkg(match), attr) is True

        # restrict that doesn't match the package
        no_match = repo_objs.FlagWithRestrict(value=True, restrict=atom.atom(">dev-util/diffball-1"))
